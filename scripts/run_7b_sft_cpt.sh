...
"$PYTHON_BIN" "$PROJECT_ROOT/tools/run_sft_stage.py" cpt --mode formal \
  --model-path "$MODEL_7B_PATH" \
  --data-dir "$PROJECT_ROOT/data/processed/sft-clean-20k-v2" \
  --cpt-adapter "$OUTPUT_ROOT/pt-full20k-7b-v2" \
  --peer-identity "$OUTPUT_ROOT/sft-base-20k-7b-v2/dataset_identity.json" \
  --output-dir "$OUTPUT_ROOT/sft-cpt-20k-7b-v2" \
